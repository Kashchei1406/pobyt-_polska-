(function () {
  ...
  function calcResult() {
    ...
    if (deport) return {
      ...
      items: ['Разберём постановление и найдём основания для обжалования',
              refusalNote,
    ...
    if (karty) return {
      ...
      body:  'Наличие карты поляка или подтверждённых польских корней открывает упрощённый путь к ВНЖ и даже к постоянному проживанию.',
      items: ['Карта поляка даёт право на ВНЖ по упрощённой процедуре',
              refusalNote,
    ...
    if (a.q4 === 'wezwanie') return {
      ...
      body:  'Wezwanie означает, что ужонд запросил документы или разъяснения по делу — без ответа в срок ситуация может осложниться.',
      items: ['Разберём запрос ужонда',
              'Определим нужные документы и форму ответа',
              'Подготовим ответный пакет',
              withFamily ? 'Учтём семью — при необходимости подготовим документы для всех' : null,
              'Уложимся в срок (часто 7–14 дней — сверим с вашим wezwanie)']
              .filter(Boolean),
      dotColor: '#BA7517',
    ...
    if (expired || overstay) return {
      ...
      alts: expired ? ['Выезд и повторный въезд — если нарушение небольшое',
                       'Национальная виза D для восстановления статуса'] : []
    };
    ...
    if (abroad) return {
      ...
      items: ['Определим основание и тип разрешения до въезда',
              refusalNote,
              'Составим точный список документов под ваше воеводство',
      ...
      alts: ['Национальная виза D — позволяет приехать уже с нужным статусом']
    };

    /* 7. 5+ лет + хорошее основание → ПМЖ */
    if (longStay && goodBase) return {
      badgeColor: '#0F6E56', badgeBg: '#E1F5EE',
      badge: 'Хорошие шансы — возможно, уже на ПМЖ',
      title: 'Вы можете претендовать на постоянный вид на жительство',
      body:  '5 лет с официальным основанием — это уже путь к ПМЖ. Постоянный статус не нужно продлевать каждые 1–2 года, а через 3 года можно подавать на гражданство.',
      items: ['Проверим стаж, доходы и возможные перерывы в пребывании',
              refusalNote,
              'ПМЖ выдаётся бессрочно — никаких регулярных продлений',
              withFamily ? 'Оформим ПМЖ сразу для всей семьи' : null,
              'Через 3 года ПМЖ можно подать на польское гражданство']
              .filter(Boolean),
      dotColor: '#0F6E56',
      cta: 'Обсудить путь к ПМЖ', alts: []
    };
    ...
    return {
      ...
      body:  'По вашим ответам основные условия для легализации есть. Следующий шаг — проверить документы и правильно собрать пакет для воеводства.',
      items: [searching    ? 'Поможем оформить основание — это первый шаг'
              ...
              'Составим точный список документов под ваше воеводство',
              familyNote,
              'Сопроводим весь процесс — от анкеты до получения карты']
              .filter(Boolean),
      dotColor: '#0F6E56',
